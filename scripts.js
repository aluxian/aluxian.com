...
(function() {
  var belowFullscreen, bgSize, fadeIn, fadeOut, fullscreen, height, i, imageSize, item, j, k, l, lastIndex, len, len1, menuItems, mouseOut, mouseOver, navList, nextIndex, phrases, popupNav, randomInt, selectedMenuItem, selectedMenuItemHighlighted, timeout, titleElement, trigger, update, x;
  bgSize = 1920;
  ...
  for (i = k = 0, len = menuItems.length; k < len; i = ++k) {
    item = menuItems[i];
  ...
  timeout = null;
  update();
  phrases = ['FULL-STACK WEB CODER', 'ANDROID DEVELOPER', 'iOS BEGINNER', 'MAC USER', 'TECHNOLOGY ENTHUSIAST', 'OCCASIONAL HACKER', 'PROBLEM SOLVER', 'YOUNG & CREATIVE', 'FRONTEND-ER', 'BACKEND-ER'];
  ...
  return smoothScroll.init({
    ...
    callbackAfter: function(toggle) {
      var len2, m;
      for (i = m = 0, len2 = menuItems.length; m < len2; i = ++m) {
        item = menuItems[i];
